# corebluetooth-rs coverage audit v2 (vs MacOSX26.2.sdk)
...
| `CBConnectionEvent` | enum | `CBCentralManager.h` | Connection-events API is unavailable on macOS; this enum is only used by macOS-unavailable delegate methods. | `CB_CM_API_AVAILABLE` = `API_UNAVAILABLE(macos)` on all consuming APIs |
| `CBCentralManagerFeature` | enum | `CBCentralManager.h` | Feature flags are only consumed by the connection-events API, which is unavailable on macOS. | `CB_CM_API_AVAILABLE` = `API_UNAVAILABLE(macos)` on `supportsFeatures:` |
| `CBConnectionEventMatchingOption` | typed enum | `CBCentralManagerConstants.h` | Typed option namespace is only used by `registerForConnectionEventsWithOptions:`, which is unavailable on macOS. | `CB_CM_API_AVAILABLE` on consuming APIs |
